...
//   rocky://open?file=Resources/Erasmus/attachments/RS.xlsx  (attachment → kind 'file')
//
// Notes (kind 'note') render on Mission Control. Anything with a known
// attachment extension (kind 'file') is handed to the OS default app instead —
// Obsidian can't open an xlsx either, so rocky:// is the only link that works
// for those.
const path = require('path');

const ATTACHMENT_EXT = new Set([
  'pdf', 'xlsx', 'xls', 'numbers', 'csv', 'docx', 'doc', 'pages', 'rtf', 'txt',
  'pptx', 'ppt', 'key', 'png', 'jpg', 'jpeg', 'gif', 'webp', 'svg', 'heic',
  'mp3', 'm4a', 'wav', 'mp4', 'mov', 'zip', 'json', 'html', 'canvas', 'tex',
]);

function attachmentExt(file) {
  const m = /\.([A-Za-z0-9]+)$/.exec(file);
  return m && ATTACHMENT_EXT.has(m[1].toLowerCase()) ? m[1].toLowerCase() : null;
}
...
function parseRockyUrl(raw) {
  ...
  if (!file) return null;
  const kind = attachmentExt(file) ? 'file' : 'note';
  if (kind === 'note' && !/\.md$/i.test(file)) file += '.md';
  ...
  return {
    ...
    heading: kind === 'note' && heading ? heading.trim() : null,
  };
}
...
module.exports = { parseRockyUrl, buildRockyUrl, attachmentExt };
